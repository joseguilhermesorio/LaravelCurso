...
<body>
    ...
    <main role="main">
      <section class="jumbotron text-center">
        <div class="container">
          @isset($msg)
            <div class="alert alert-success"><h1>{{ $msg }}</h1></div>
          @endif
          <h1 class="jumbotron-heading">Escreva aqui seu novo Post</h1>
          <form method="POST" action="{{ route('criarpostagem') }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group text-left">
              <label for="email">Endereço de e-mail</label>
              <input type="email" class="form-control" id="email" name="email" placeholder="nome@dominio.com" value="{{ old('email') }}">
            ...
            <div class="custom-file">
              ...
              @if($errors->has('arquivo'))
                <span style="color: red;">{{ $errors->first('arquivo') }}</span>
              @endif
            </div>
            <p>
                    ...
                    <div class="card-body">
                      ...
                      <div class="d-flex justify-content-between align-items-center">
                        <div class="btn-group">
                          ...
                          <a type="button" class="btn btn-sm btn-outline-secondary" href="{{ route('downloadarquivo', ['id' => $post->id]) }}">Download</a>
                          <form method="post" action="{{ route('deletarpost',['id' => $post->id]) }}">
                            @csrf
    ...
    <footer class="text-muted">
      <div class="container">
        ...
        <p>{{ date('Y') }} jose guilherme sorio</p>
      </div>
    </footer>
